...
use App\Jobs\Reports\ProcessLostClientsReportJob;
use App\Models\Partner\Traits\HasPartnerReportDispatch;
use Illuminate\Console\Command;

final class SendLostClientsReportCommand extends Command
{
    use HasPartnerReportDispatch;

    protected $signature = 'report:lost-clients
                            {--company_id= : Конкретный ID компании из YClients (yclients_id)}';

    protected $description = 'Запуск рассылки отчета по потерянным клиентам';
    ...
    public function handle(): int
    {
        ...
        foreach ($partners as $partner) {
            ProcessLostClientsReportJob::dispatch($partner);
