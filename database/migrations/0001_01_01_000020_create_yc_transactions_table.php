...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('yc_staff_transactions', function (Blueprint $table) {
            $table->unsignedBigInteger('transaction_id')->primary();

            $table->unsignedBigInteger('company_id');

            // Сотрудник по которому делам выгрузку
            $table->unsignedBigInteger('staff_id')->nullable();

            // ID мастера который указан в YC
            $table->unsignedBigInteger('master_id')->nullable();
            ...
            $table->index(['company_id', 'date'], 'yc_trans_company_date_idx');
            $table->index(['company_id', 'staff_id', 'date'], 'yc_trans_staff_date_idx');
            $table->index('record_id');
            $table->index('visit_id');
            $table->index('account_id');
        });
    ...
    {
        Schema::dropIfExists('yc_staff_transactions');
